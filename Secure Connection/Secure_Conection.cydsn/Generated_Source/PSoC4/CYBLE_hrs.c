...
const CYBLE_HRSS_T cyBle_hrss =
{
    0x000Cu,    /* Handle of the HRS service */
    {
        ...
        CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE, /* Handle of the Body Sensor Location characteristic */ 
        CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE, /* Handle of the Heart Rate Control Point characteristic */ 
    },
                                                        ...
                                                        uint8 attrSize, uint8 * attrValue)
{
	...
	else if(CYBLE_GATT_DB_INVALID_HANDLE == cyBle_hrsc.charInfo[charIndex].valueHandle)
    {
		apiResult = CYBLE_ERROR_GATT_DB_INVALID_ATTR_HANDLE;
	}
    ...
    CYBLE_HRS_CHAR_INDEX_T charIndex, CYBLE_HRS_DESCR_INDEX_T descrIndex, uint8 attrSize, uint8 * attrValue)
{
    ...
    else if(CYBLE_GATT_DB_INVALID_HANDLE == cyBle_hrsc.hrmCccdHandle)
    {
		apiResult = CYBLE_ERROR_GATT_DB_INVALID_ATTR_HANDLE;
	}
    else
...
void CyBle_HrscDiscoverCharacteristicsEventHandler(CYBLE_DISC_CHAR_INFO_T *discCharInfo)
{
    ...
    for(i = 0u; i < (uint8) CYBLE_HRS_CHAR_COUNT; i++)
    {
        ...
        {
            if(cyBle_hrsc.charInfo[i].valueHandle == CYBLE_GATT_DB_INVALID_HANDLE)
            {
...
void CyBle_HrscDiscoverCharDescriptorsEventHandler(CYBLE_DISC_DESCR_INFO_T * discDescrInfo)
{
    ...
    {
        if(cyBle_hrsc.hrmCccdHandle == CYBLE_GATT_DB_INVALID_HANDLE)
        {
