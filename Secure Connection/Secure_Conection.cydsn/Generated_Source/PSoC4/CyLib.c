...
void CySysClkIloStop(void)
{
        ...
        if (!CySysWdtLocked() && CySysClkIloEnabled())
        ...
        } /* Otherwise do nothing. ILO configuration cannot be changed. */
    #else
        CY_SYS_CLK_ILO_CONFIG_REG &= ( uint32 ) ( ~( uint32 )CY_SYS_CLK_ILO_CONFIG_ENABLE);
    #endif /* (CY_IP_SRSSV2) */
}
    ...
    *
    * Return:
    *  True if ILO is enabled, and false if ILO is disabled.
    *
    *******************************************************************************/
    static uint32 CySysClkIloEnabled(void)
    {
...
void CySysClkWriteSysclkDiv(uint32 divider)
{
    ...
    *******************************************************************************/
    void  CySysClkWcoStop(void)
    {
        ...
        if (!CySysWdtLocked() && CySysClkWcoEnabled())
        {   /* Do nothing if WDT is locked or WCO is disabled */
    ...
    *
    * Return:
    *  True if WCO is enabled, and false if WCO is disabled.
    *
    *******************************************************************************/
    static uint32 CySysClkWcoEnabled(void)
    {
    ...
    *
    * Return:
    *  False - WDT is unlocked, true - WDT is locked.
    *
    *******************************************************************************/
    static uint32 CySysWdtLocked(void)
    {
        /* Prohibits writing to WDT registers and ILO/WCO registers when not equal 0 */
        return (0u != (CY_SYS_CLK_SELECT_REG & (uint32)(CY_SYS_WDT_CLK_LOCK_BITS_MASK)));
    }
