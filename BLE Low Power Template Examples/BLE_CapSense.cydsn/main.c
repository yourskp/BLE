...
CAPSENSE_STATE_T capSenseState = SCAN_SENSOR;
...
void ManageApplicationPower()
{
    switch(applicationPower)
    {
        ...
        case WAKEUP_DEEPSLEEP: // do whatever wakeup needs to be done.
            if(capSenseState == SHUTDOWN && CyBle_GetBleSsState() != CYBLE_BLESS_STATE_DEEPSLEEP)
            {
                CapSense_State_Write(CapSense_State_Read() ^ 0x01);
                CapSense_Wakeup();
                capSenseState = SCAN_SENSOR;
                applicationPower = ACTIVE;
            }
            else
            {
    ...
    ************************************************************************/ 
        if(capSenseState != SHUTDOWN)
        {
            CapSense_State_Write(CapSense_State_Read() ^ 0x01);
            CapSense_Sleep();
            capSenseState = SHUTDOWN;
        }
        break;
    }
}
...
inline void RunApplication()
{
    ...
    ************************************************************************/  
    switch(capSenseState)
    {
        case SCAN_SENSOR:
            CapSense_State_Write(CapSense_State_Read() ^ 0x01);
            CapSense_ScanEnabledWidgets();
            capSenseState = WAIT_FOR_SCAN_COMPLETE;
        ...
        case PROCESS_DATA:
            ...
            centroid = CapSense_GetCentroidPos(CapSense_LINEARSLIDER0__LS);
            CapSense_State_Write(CapSense_State_Read() ^ 0x01);
            if(centroid != 0xFFFF)
            {
                UART_PutHexByte((uint8) centroid);
                UART_PutString("\r\n");
            }
            capSenseState = INACTIVE;
            applicationPower = DEEPSLEEP;
        break;
