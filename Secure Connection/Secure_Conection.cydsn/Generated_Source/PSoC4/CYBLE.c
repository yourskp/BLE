...
***************************************/
        ...
        static void CyBle_ChangeAdLocalName(const char8 name[]);
...
uint8 cyBle_stackMemoryHeap[CYBLE_STACK_HEAP_SIZE];
...
{
    { 0x02u, 0x01u, 0x06u, 0x0Bu, 0x09u, 0x53u, 0x65u,
    0x63u, 0x75u, 0x72u, 0x65u, 0x20u, 0x48u, 0x52u, 0x4Du,
    0x05u, 0x03u, 0x0Du, 0x18u, 0x0Au, 0x18u, 0x03u, 0x19u,
    0x40u, 0x03u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u }, /* uint8 advertising_data[CYBLE_MAX_ADV_DATA_LEN] */
    0x19u,      /* uint8 adv_data_length */
};

CYBLE_GAPP_SCAN_RSP_DATA_T cyBle_scanRspData =
{
    { 0x05u, 0x03u, 0x0Du, 0x18u, 0x0Au, 0x18u, 0x00u,
...
void CyBle_Init(void)
{
	...
	(void) memcpy (cyBle_attValuesCCCD, cyBle_attValuesCCCDFlashMemory, CYBLE_GATT_DB_CCCD_COUNT);
#endif /* CYBLE_GATT_ROLE_SERVER */
...
CYBLE_API_RESULT_T CyBle_Start(CYBLE_CALLBACK_T callbackFunc)
{
    ...
    if(NULL != callbackFunc)
    {
        ...
        apiResult = CyBle_StackInit(&CyBle_EventHandler, cyBle_stackMemoryHeap, CYBLE_GATT_MTU);
...
Summary:
 ...
 This function is available only when Bounding requirement is selected in
 Security settings.    
...
CYBLE_API_RESULT_T CyBle_StoreBondingData(uint8 isForceWrite)
{
    ...
    {
        apiResult = CyBle_StoreAppData(cyBle_attValuesCCCD, cyBle_attValuesCCCDFlashMemory, CYBLE_GATT_DB_CCCD_COUNT, 
                                       isForceWrite);
        if(apiResult == CYBLE_ERROR_OK)
        {
    ...
    *******************************************************************************
     ...
     * CYBLE_EVT_GAP_DEVICE_CONNECTED
     ...
     A procedure is considered to have timed out if a connection reponce packet is 
    ...
    ******************************************************************************/
    CYBLE_API_RESULT_T CyBle_GapcCancelDeviceConnection(void)
    {
        ...
        *******************************************************************************
        Summary:
         This API is used to set the local device name in the advertisement data 
         structure.
        ...
        ******************************************************************************/
        static void CyBle_ChangeAdLocalName(const char8 name[])
        {
            ...
            while((byteCounter < CYBLE_AD_STRUCTURE_MAX_LENGTH) && (0u == flag))
            {
                ...
                if(0u != adLength)
                {
                    ...
                    if((CYBLE_SHORT_LOCAL_NAME == cyBle_discoveryData.advData[byteCounter]) || 
                        (CYBLE_COMPLETE_LOCAL_NAME == cyBle_discoveryData.advData[byteCounter]))
                    {
            ...
            if(0u != flag)
            {
                /* Reuse "adLength" to hold location of the last character of local name in 
                   AD structure. */
                adLength = (adLength + byteCounter) - 1u;
                ...
                {
                    cyBle_discoveryData.advData[i] = (uint8)name[i - byteCounter];
                }
                ...
                {
                    /* Terminate string */
                    cyBle_discoveryData.advData[i] = (uint8)CYBLE_NULL_CHARCTER;
                    i++;
                }
            }
        }
